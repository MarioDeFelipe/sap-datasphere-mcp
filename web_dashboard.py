...
async def get_assets(source_system: Optional[str] = None, asset_type: Optional[str] = None):
    """Get assets from connected systems"""
    try:
        all_assets = []
        
        # Fetch assets from Datasphere if connector is available
        if datasphere_connector and (not source_system or source_system == "datasphere"):
            try:
                print("🔍 Fetching assets from Datasphere...")
                # Use the correct method to get assets
        ...
        # Fetch assets from AWS Glue if connector is available
        if glue_connector and (not source_system or source_system == "glue"):
            try:
                print("🔍 Fetching assets from AWS Glue...")
                # Use the correct method to get assets
                ...
                print(f"✅ Found {len(glue_assets)} assets from AWS Glue")
                
            except Exception as e:
                print(f"⚠️ Failed to fetch AWS Glue assets: {str(e)}")
        
        # If no real connectors available, return sample data with clear indication
        if not all_assets and not datasphere_connector and not glue_connector:
            print("⚠️ No connectors available, returning sample data")
            all_assets = [
                {
                    "asset_id": "sample_financial_model",
                    "asset_type": "analytical_model",
                    "source_system": "datasphere",
                    "technical_name": "SAMPLE_FINANCIAL_MODEL",
                    "business_name": "Sample Financial Model (Demo Data)",
                    "description": "Sample data - Connect to real Datasphere to see actual assets",
                    "owner": "demo_user",
                    "last_modified": (datetime.now() - timedelta(hours=2)).isoformat(),
                    "sync_status": "demo"
                },
                {
                    "asset_id": "sample_customer_table",
                    "asset_type": "table",
                    "source_system": "glue",
                    "technical_name": "sample_customer_table",
                    "business_name": "Sample Customer Table (Demo Data)",
                    "description": "Sample data - Connect to real AWS Glue to see actual assets",
                    "owner": "demo_user",
