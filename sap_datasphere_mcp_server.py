...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description=enhanced["create_sql_view"]["description"],
            inputSchema=enhanced["create_sql_view"]["inputSchema"]
        ),
        Tool(
...
<edmx:Edmx xmlns:edmx="http://docs.oasis-open.org/odata/ns/edmx" Version="4.0">
  <edmx:DataServices>
    ...
    # View Management Tools
        ...
        # If no columns provided, fetch source schema to pass all through
        if not columns:
            if not datasphere_connector and not DATASPHERE_CONFIG["use_mock_data"]:
                return [types.TextContent(
                    type="text",
                    text="No columns specified and no connection to fetch source schema.\n"
                         "Please provide 'columns' array or configure OAuth credentials."
                )]
            # Try to fetch source columns from the CLI
            try:
                ...
                if src_read.returncode == 0:
                    src_def = json.loads(src_read.stdout)
                    ...
                    return [types.TextContent(
                        type="text",
                        text=f"Could not read source object '{source_object}' schema.\n"
        ...
        for col in columns:
            ...
            query_columns.append(col_entry)
        ...
        for col in columns:
            ...
            # Semantic annotation: dimension vs measure
            if col.get("measure"):
        ...
        # Build the full CSN definition matching Datasphere's expected format
        view_definition = {
            "kind": "entity",
        ...
        # Build editorSettings so Datasphere opens this as a graphical view
        # Node chain: Entity → Projection → Output
        # Each node links via successorNode, each element links via successorElement
        import uuid
        ...
        # Map dataCategory for the Output node
        data_category_map = {
            "FACT": "SQLFACT",
            "DIMENSION": "DIMENSION",
            "TEXT": "TEXT",
        ...
        # Build element entries for each node in the chain:
        # Entity elements → (successorElement) → Projection elements → (successorElement) → Output elements
        output_elements = {}
        projection_elements = {}
        entity_elements = {}
        ui_element_contents = {}
        ...
        for i, col in enumerate(columns):
            ...
            # Node element refs (just name pointers)
            ...
            # Output element content (end of chain, isCalculated=True)
            ...
            # Projection element content (middle of chain)
            ...
            # Entity element content (start of chain)
        ...
        # Assemble uiModel contents
        contents = {}

        # Model (root) — registers all nodes and diagram
        contents[model_id] = {
        ...
        # Projection node (middle) — successorNode → Output
        ...
        # Entity node (source) — successorNode → Projection
        contents[entity_id] = {
        ...
        # Diagram (UI layout — symbols auto-position)
        contents[diagram_id] = {
        ...
        if DATASPHERE_CONFIG["use_mock_data"]:
            result = {
                "status": "SUCCESS (mock)",
                "message": f"View '{view_id}' would be created in space '{space_id}'",
                "csn_definition": csn_definition,
                "note": "Mock mode. Set USE_MOCK_DATA=false for real view creation."
            }
            return [types.TextContent(
                type="text",
                text=f"View Preview:\n\n{json.dumps(result, indent=2)}"
            ...
            # Real mode - use datasphere CLI to create the view
            try:
                import subprocess
                import tempfile
                import os

                logger.info(f"Creating graphical view {view_id} in space {space_id}")

                # Write CSN definition to temporary JSON file
                with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
                    json.dump(csn_definition, temp_file, indent=2)
                    temp_file_path = temp_file.name

                logger.info(f"CSN definition: {json.dumps(csn_definition, indent=2)}")

                try:
                    # Execute datasphere CLI: objects views create
                    cmd = [
                    ...
                    result = {
                        ...
                        "next_steps": [
                            "Use get_object_definition to verify the view",
                            "Deploy the view to make it queryable" if not deploy else "View is deployed and queryable",
                        ]
                    }
                ...
                error_detail = e.stderr.strip() or e.stdout.strip() or "No error details"
                logger.error(f"CLI error creating view: {error_detail}")
                return [types.TextContent(
                    type="text",
                    text=f"Error creating view:\n\n"
                ...
                return [types.TextContent(
                    ...
                    text="Error: 'datasphere' CLI not found.\n\n"
                         ...
                         "Then login with: datasphere login"
                )]
            except Exception as e:
        ...
        join_conditions = arguments.get("join_conditions", [])
        # UNION params
        union_object = arguments.get("union_object", "")
        union_all = arguments.get("union_all", True)
        ...
        if node_type == "JOIN" and (not join_object or not join_conditions):
            return [types.TextContent(
                type="text",
                text="join_object and join_conditions are required when node_type is JOIN."
            )]

        if node_type == "UNION" and not union_object:
            return [types.TextContent(
                type="text",
        ...
        try:
            ...
            logger.info(f"Reading existing view {view_id} from space {space_id}")
            ...
            if read_proc.returncode != 0:
                return [types.TextContent(
                    type="text",
                    text=f"Could not read view '{view_id}': {read_proc.stderr or read_proc.stdout}"
                )]

            view_data = json.loads(read_proc.stdout)
            view_def = view_data["definitions"][view_id]
            editor_settings = view_data.get("editorSettings", {})

            if node_type == "FILTER":
                # Step 2a: Update CSN — add WHERE clause to query
                import re as _re
                # Parse filter condition, respecting quoted strings
                # Tokenize: quoted strings kept whole, then words
                ...
                while i < len(raw_tokens):
                    ...
                    if token_upper in ("AND", "OR"):
                        ...
                        if source_ref:
                            csn_where.append({"ref": [source_ref, col_name]})
                        else:
                            csn_where.append({"ref": [col_name]})

                        csn_where.append(operator)

                        try:
                            csn_where.append({"val": int(value)})
                        except ValueError:
                            try:
                                csn_where.append({"val": float(value)})
                            except ValueError:
                                csn_where.append({"val": value})
                ...
                # Step 2b: Update uiModel — insert Filter node before Output
                ...
                if ui_model_str:
                    ...
                    if output_node_id and output_content:
                        # Find predecessor node (points to Output)
                        predecessor_id = None
                        for cid, content in contents.items():
                        ...
                        predecessor_content = contents.get(predecessor_id, {})
                        ...
                        # Build filter elements from predecessor elements
                        for pred_el_id, pred_el_ref in pred_els.items():
                            ...
                            # Repoint predecessor element to filter element
                        ...
                        contents[filter_node_id] = {
                            ...
                            "successorNode": output_node_id,
                        }

                        # Repoint predecessor node to filter
                        if predecessor_id:
                        ...
                        # Add filter element contents
                        contents.update(filter_element_contents)

                        # --- Diagram symbols ---
                        if diagram_content:
                            # Clean up orphan galilei symbols (created by CLI)
                            ...
                            for sid, sym in contents.items():
                                if sym.get("object") == output_node_id:
                                    out_sym_id = sid
                                    out_sym = sym
                            ...
                            if out_sym and pre_out_sym:
                                # Place new node where output was, shift output right
                                new_x = out_sym.get("x", 0)
                                ...
                                # Association: predecessor → filter
                                assoc1_id = _new_id()
                                contents[assoc1_id] = {
                                ...
                                contents[assoc2_id] = {
                                    ...
                                    "targetSymbol": out_sym_id,
                                }
                                diagram_content["symbols"][assoc2_id] = {}

                                # Remove old direct association (predecessor → output)
                ...
                # Step 2a: Update CSN — add element + column entry (no xpr needed)
                ...
                columns_list = view_def.get("query", {}).get("SELECT", {}).get("columns", [])
                columns_list.append({"as": column_name})
                view_def["query"]["SELECT"]["columns"] = columns_list

                # Step 2b: Update uiModel — insert CalculatedElements node before Output
                ui_model_str = editor_settings.get(view_id, {}).get("uiModel")
                if ui_model_str:
                    ui_model = json.loads(ui_model_str)
                    contents = ui_model["contents"]
                    ...
                    if output_content and proj_content:
                        # Find the node whose successorNode points to Output
                        pre_output_id = None
                        for cid, content in contents.items():
                        ...
                        # --- Create Output element for new column ---
                        out_el_id = _new_id()
                        ...
                        # --- Create CalculatedElements node ---
                        calc_node_id = _new_id()
                        calc_elements = {}
                        calc_element_contents = {}

                        # Pass-through elements: copy from the pre-output node's elements
                        pre_output_content = contents.get(pre_output_id, {})
                        ...
                        for pre_el_id, pre_el_ref in pre_output_els.items():
                            ...
                            # Create calc element for this pass-through column
                            calc_el_id = _new_id()
                            ...
                            # Repoint pre-output element to this calc element
                            pre_el_detail["successorElement"] = calc_el_id

                        # Add the new calculated column element to the calc node
                        ...
                        # Insert calc node into contents
                        ...
                        # Repoint pre-output node to calc node
                        if pre_output_id:
                            contents[pre_output_id]["successorNode"] = calc_node_id

                        # Add calc element contents
                        contents.update(calc_element_contents)

                        # Register calc node in Model
                        if model_entry:
                            model_entry["nodes"][calc_node_id] = {"name": node_name}

                        # Add the new column element to Projection node with expression
                        proj_el_id = _new_id()
                        proj_content["elements"][proj_el_id] = {"name": column_name}
                        # Find the calc element for this column to chain to
                        contents[proj_el_id] = {
                            "classDefinition": "sap.cdw.querybuilder.Element",
                            "name": column_name,
                            "label": label,
                        ...
                        if diagram_content:
                            ...
                            pre_out_sym = None
                            for sid, sym in contents.items():
                                if sym.get("object") == output_id:
                                    out_sym_id = sid
                                    out_sym = sym
                                elif sym.get("object") == pre_output_id:
                                    pre_out_sym_id = sid
                                    pre_out_sym = sym
                            ...
                            if out_sym and pre_out_sym:
                                ...
                                # Remove old direct association (pre-output → output)
                                old_assoc_ids = [sid for sid, sym in contents.items()
                                                 if sym.get("classDefinition") == "sap.cdw.querybuilder.ui.AssociationSymbol"
                                                 and sym.get("sourceSymbol") == pre_out_sym_id
                                                 and sym.get("targetSymbol") == out_sym_id]
                ...
                # Step 2a: Update CSN — change FROM to join structure
                source_ref = None
                query_from = view_def.get("query", {}).get("SELECT", {}).get("from", {})
                if isinstance(query_from, dict) and "ref" in query_from:
                    source_ref = query_from["ref"][0]
                ...
                if not source_ref:
                    return [types.TextContent(
                        type="text",
                        text="Cannot add JOIN: view must have a simple source (FROM ref). Nested joins not yet supported."
                    )]
                ...
                # Replace FROM with join structure
                ...
                # Update WHERE refs to use left alias
                ...
                if ui_model_str:
                    ...
                    entity_content = None
                    diagram_content = None
                    # Find first node after entity (the one entity currently points to)
                    ...
                    if entity_content and first_successor_id:
                        # Add alias to existing entity
                        entity_content["alias"] = left_alias

                        # Fetch right source columns via CLI
                        right_cols_proc = subprocess.run(
                            ["datasphere", "objects", "views", "read",
                             "--space", space_id, "--technical-name", join_object],
                            capture_output=True, text=True, timeout=60
                        )
                        right_source_elements = {}
                        if right_cols_proc.returncode == 0:
                            right_data = json.loads(right_cols_proc.stdout)
                            right_def = right_data.get("definitions", {}).get(join_object, {})
                            right_source_elements = right_def.get("elements", {})
                            # Also try editorSettings uiModel for element details
                        ...
                        for col_name, col_def in right_source_elements.items():
                            ...
                            ui_detail = right_ui_elements.get(col_name, {})
                        ...
                        # Build Join elements: all left entity elements + all right entity elements
                        join_elements = {}
                        join_element_contents = {}

                        # Left entity elements pass through to join
                        left_els = entity_content.get("elements", {})
                        # Map by name for join condition matching
                        left_el_by_name = {}
                        ...
                        # Right entity elements pass through to join
                        right_el_by_name = {}
                        ...
                        # Build mappings (join condition element pairs)
                        join_mappings = {}
                        ...
                        # Wire successor elements: Join elements → first successor's elements (by name)
                        ...
                        # Repoint left entity elements: entity → join elements (by name)
                        join_el_by_name = {}
                        for jel_id, jel_ref in join_elements.items():
                            # Use first match (left elements come first)
                        ...
                        # Right entity elements → join elements (by name, second match)
                        # Build map of right-side join elements
                        right_join_el_by_name = {}
                        seen_names = set()
                        for jel_id, jel_ref in join_elements.items():
                            name = jel_ref["name"]
                            if name in seen_names:
                                right_join_el_by_name[name] = jel_id
                            seen_names.add(name)
                        ...
                        # Insert right entity node
                        contents[right_entity_id] = {
                            "classDefinition": "sap.cdw.querybuilder.Entity",
                            "name": join_object,
                            "label": join_object,
                            "type": 3,
                            "isDeltaOutboundOn": entity_content.get("isDeltaOutboundOn", False),
                            "isPinToMemoryEnabled": False,
                            "dataCategory": entity_content.get("dataCategory", "DIMENSION"),
                            "isUseOLAPDBHint": entity_content.get("isUseOLAPDBHint", False),
                            "isHiddenInUi": False,
                            "alias": right_alias,
                        ...
                        # Repoint left entity to join
                        entity_content["successorNode"] = join_node_id
                        ...
                        # --- Diagram symbols ---
                        if diagram_content:
                            # Clean up orphan galilei symbols
                            orphan_ids = [cid for cid, c in contents.items()
                                          if c.get("classDefinition") == "sap.galilei.ui.diagram.Symbol"]
                            for oid in orphan_ids:
                                contents.pop(oid)
                            ...
                            # Find existing entity symbol
                            entity_sym_id = None
                            entity_sym = None
                            first_succ_sym_id = None
                            first_succ_sym = None
                            for sid, sym in contents.items():
                                if sym.get("object") == entity_id:
                                    entity_sym_id = sid
                                    entity_sym = sym
                            ...
                            if entity_sym:
                                ent_x = entity_sym.get("x", 0)
                                ent_y = entity_sym.get("y", 0)
                                ent_w = entity_sym.get("width", 168)

                                # Shift left entity down, place right entity above
                                entity_sym["y"] = ent_y + 45
                                ...
                                # Place join symbol after entities, before first successor
                                ...
                                contents[assoc_left_id] = {
                                    ...
                                    "contentOffsetX": 5,
                                    "contentOffsetY": 5,
                                    "sourceSymbol": entity_sym_id,
                                    "targetSymbol": join_sym_id,
                                    "object": join_node_id,
                                }
                                diagram_content["symbols"][assoc_left_id] = {}

                                # Association: right entity → join (isLeftInput: false)
                                ...
                                # Association: join → first successor
                                ...
                                # Remove old direct association (entity → first successor)
                                old_assoc_ids = [sid for sid, sym in contents.items()
                                                 if sym.get("classDefinition") == "sap.cdw.querybuilder.ui.AssociationSymbol"
                                                 and sym.get("sourceSymbol") == entity_sym_id
                ...
                # Step 2a: Update CSN — wrap FROM in a SET (union) structure
                existing_from = view_def.get("query", {}).get("SELECT", {}).get("from", {})

                # Detect FROM type: simple ref or join or other
                source_ref = None  # only set for simple ref case
                if isinstance(existing_from, dict) and "ref" in existing_from:
                    source_ref = existing_from["ref"][0]
                    left_from = {"ref": [source_ref]}
                elif isinstance(existing_from, dict) and "join" in existing_from:
                    # Join case — use entire join structure as left FROM
                    left_from = existing_from
                else:
                    return [types.TextContent(
                        type="text",
                        text="Cannot add UNION: unsupported FROM structure. Only simple ref and join are supported."
                    )]
                ...
                # Use the view's own output elements as the column list
                view_elements = view_def.get("elements", {})
                right_alias = f"{union_object}(2)"

                # Build left SELECT columns from existing outer SELECT columns
                existing_cols = view_def["query"]["SELECT"].get("columns", [])
                left_columns = []
                if source_ref:
                    # Simple ref: columns reference the source entity
                    for col_name in view_elements:
                        left_columns.append({"ref": [source_ref, col_name]})
                else:
                    # Join/complex: copy refs from existing outer SELECT columns
                    for col in existing_cols:
                        if "ref" in col:
                            left_col = {"ref": list(col["ref"])}
                            if "as" in col:
                                left_col["as"] = col["as"]
                            left_columns.append(left_col)

                # Build right SELECT columns
                right_columns = []
                for col_name in view_elements:
                ...
                union_set = {
                    "SET": {
                        ...
                        "args": [
                            ...
                            {
                                "SELECT": {
                                    "from": {"ref": [union_object], "as": right_alias},
                ...
                # Replace the FROM
                view_def["query"]["SELECT"]["from"] = union_set

                # Update ALL existing column refs to use union alias (node_name)
                # After union, all outer columns reference the union node regardless of original source
                for col in view_def["query"]["SELECT"].get("columns", []):
                    if "ref" in col and len(col["ref"]) == 2:
                        col["ref"][0] = node_name
                    if "xpr" in col:
                        for item in col["xpr"]:
                            if isinstance(item, dict) and "ref" in item:
                                if len(item["ref"]) == 2:
                                    item["ref"][0] = node_name
                ...
                if ui_model_str:
                    ...
                    # Find key nodes
                    model_entry = None
                    all_entities = []  # collect all Entity nodes
                    diagram_content = None
                    ...
                    for cid, content in contents.items():
                        ...
                        if cls == "sap.cdw.querybuilder.Model":
                            ...
                            all_entities.append((cid, content))
                        elif cls == "sap.cdw.querybuilder.ui.Diagram":
                            diagram_content = content

                    # Pick the primary entity: for simple ref match by name, otherwise first entity
                    entity_id = None
                    entity_content = None
                    first_successor_id = None
                    if source_ref:
                        for eid, econt in all_entities:
                            if econt.get("name") == source_ref:
                                entity_id = eid
                                entity_content = econt
                                first_successor_id = econt.get("successorNode")
                                break
                    if not entity_content and all_entities:
                        # Fallback: use first entity
                        entity_id, entity_content = all_entities[0]
                        first_successor_id = entity_content.get("successorNode")

                    if entity_content and first_successor_id:
                        # Create second Entity node for union source
                        right_entity_id = _new_id()
                        right_entity_els = {}
                        right_entity_el_details = {}
                        idx = 0
                        ...
                        # Create Union node with pass-through elements from source
                        union_node_id = _new_id()
                        union_elements = {}
                        union_element_contents = {}

                        # Union elements match the source columns
                        left_els = entity_content.get("elements", {})
                        for el_id, el_ref in left_els.items():
                            union_el_id = _new_id()
                            union_elements[union_el_id] = {"name": el_ref["name"]}
                            el_detail = contents.get(el_id, {})
                            ...
                            if el_detail.get("dataType"):
                                union_element_contents[union_el_id]["dataType"] = el_detail["dataType"]

                        # Wire union elements → first successor elements (by name)
                        first_succ_content = contents.get(first_successor_id, {})
                        first_succ_els = first_succ_content.get("elements", {})
                        first_succ_el_by_name = {}
                        for fs_el_id, fs_el_ref in first_succ_els.items():
                            first_succ_el_by_name[fs_el_ref["name"]] = fs_el_id

                        for u_el_id, u_el_ref in union_elements.items():
                            target = first_succ_el_by_name.get(u_el_ref["name"])
                            if target:
                                union_element_contents[u_el_id]["successorElement"] = target

                        # Repoint entity elements → union elements (by name)
                        union_el_by_name = {}
                        for u_el_id, u_el_ref in union_elements.items():
                        ...
                        for el_id, el_ref in left_els.items():
                            el_detail = contents.get(el_id, {})
                            target = union_el_by_name.get(el_ref["name"])
                            if target:
                                el_detail["successorElement"] = target

                        # Right entity elements → union elements
                        for rel_id, rel_ref in right_entity_els.items():
                            rel_detail = right_entity_el_details.get(rel_id, {})
                            target = union_el_by_name.get(rel_ref["name"])
                            if target:
                                rel_detail["successorElement"] = target
                        ...
                        # Repoint entity to union
                        entity_content["successorNode"] = union_node_id
                        ...
                        if diagram_content:
                            ...
                            if entity_sym:
                                ...
                                # Shift entity down, place right entity above
                                entity_sym["y"] = ent_y + 45
                                right_ent_sym_id = _new_id()
                                contents[right_ent_sym_id] = {
                                    "classDefinition": "sap.cdw.querybuilder.ui.EntitySymbol",
                                ...
                                # Union symbol: place where first successor is, shift successor right
                                union_x = ent_x + ent_w + 50
                                union_y = ent_y
                                if first_succ_sym_id:
                                    fs_sym = contents.get(first_succ_sym_id, {})
                                    union_x = fs_sym.get("x", union_x)
                                    union_y = fs_sym.get("y", union_y)
                                    # Shift first successor and everything after it 98px right
                                    shift_ids = set()
                                    shift_ids.add(first_succ_sym_id)
                                    # Find all symbols to the right of the union position
                                    for sid2, sym2 in contents.items():
                                        if sym2.get("classDefinition", "").startswith("sap.cdw.querybuilder.ui.") \
                                                ...
                                                and sid2 != entity_sym_id and sid2 != right_ent_sym_id \
                                ...
                                # Association: right entity → union
                                assoc2_id = _new_id()
                                contents[assoc2_id] = {
                                    "classDefinition": "sap.cdw.querybuilder.ui.AssociationSymbol",
                                    "points": "",
                                ...
                                # Association: union → first successor
                                assoc3_id = _new_id()
                                contents[assoc3_id] = {
                                    "classDefinition": "sap.cdw.querybuilder.ui.AssociationSymbol",
                                    "points": "",
            ...
            # Step 3: Write updated definition and save via CLI
            updated_data = {
                "definitions": {view_id: view_def},
                "editorSettings": editor_settings,
            }
            # Preserve other top-level keys (sharing, etc.)
            ...
            try:
                ...
                logger.info(f"Updating view {view_id}: {' '.join(cmd)}")
                result_proc = subprocess.run(
            ...
            error_detail = e.stderr.strip() or e.stdout.strip() or "No error details"
            logger.error(f"CLI error updating view: {error_detail}")
            return [types.TextContent(
                type="text",
                text=f"Error updating view:\n\n"
                     f"Command: {' '.join(e.cmd)}\n"
                     f"Exit code: {e.returncode}\n"
                     f"Error: {error_detail}"
            )]
