...
def test_hostile_asset_id_rejected(validator, value):
    ok, errors = validator.validate_params(
        {"space_id": "SCM_BUSINESS", "asset_id": value, "entity_name": "E"},
...
@pytest.mark.parametrize(
    "tool,params",
    [
        ("run_task_chain", {"space_id": "SCM_BUSINESS", "object_id": "CHAIN/../x"}),
        ("get_task_history", {"space_id": "SCM_BUSINESS", "object_id": "../x"}),
        ("get_asset_variables", {"space_id": "SCM_BUSINESS", "asset_id": "../x"}),
        ("list_relational_entities", {"space_id": "SCM_BUSINESS", "asset_id": "a/b"}),
        ("get_relational_odata_service", {"space_id": "SCM_BUSINESS", "asset_id": "a?b"}),
    ],
)
...
def test_legitimate_identifiers_still_accepted(validator):
    ok, errors = validator.validate_params(
        {
            "space_id": "SCM_BUSINESS",
            "asset_id": "OPEN_PO_DETAIL",
            "entity_name": "OPEN_PO_DETAIL",
            "top": 100,
        },
        ToolValidators.get_validator_rules("query_relational_entity"),
    )
...
def test_real_world_asset_names_accepted(validator):
    """Names taken from the live Parts Town catalog."""
    for asset in ["ZRET_Open_Lines", "PR_Report_DJM", "ZD_MATERIAL", "OPEN_PO_DETAIL"]:
        ok, errors = validator.validate_params(
            {"space_id": "SCM_BUSINESS", "asset_id": asset},
...
def test_allowed_values_enforced_on_string_rules(validator, tool, param, good, bad):
    ...
    """
    base = {
        "smart_query": {"space_id": "SCM_BUSINESS", "query": "x"},
        "get_task_log": {"space_id": "SCM_BUSINESS", "log_id": 1},
...
def test_seg_is_a_noop_for_legitimate_identifiers():
    ...
    for value in ["SCM_BUSINESS", "OPEN_PO_DETAIL", "ZRET_Open_Lines", "ZD_MATERIAL"]:
