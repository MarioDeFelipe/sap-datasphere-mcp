...
Verified as a no-op for real identifiers (`SCM_BUSINESS`, `OPEN_PO_DETAIL`,
`ZRET_Open_Lines`, `ZD_MATERIAL` are unchanged) and confirmed live against the
Parts Town tenant: the quoted path returns rows normally.
