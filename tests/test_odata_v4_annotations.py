...
Validates that the V4 annotation parser:
  * extracts ``Common.Label`` strings from external ``<Annotations Target=…>``
    ...
    (SCM_BUSINESS/OPEN_PO_DETAIL captured live from partstown-1, 2026-06-27);
...
# ─────────────────────────────────────────────────────────────────────────────
# Real-fixture tests — SCM_BUSINESS / OPEN_PO_DETAIL on partstown-1
# ─────────────────────────────────────────────────────────────────────────────


def test_relational_fixture_labels_non_empty():
    root = _load("odata_v4_relational_OPEN_PO_DETAIL.xml")
    get_sem = make_semantics_extractor(root, NS)
    entity_type = root.find(".//edm:EntityType", NS)
    assert entity_type.get("Name") == "OPEN_PO_DETAIL"
    ...
    assert labels["EBELN"] == "Purchasing Document Number"
    assert labels["EBELP"] == "Item Number of Purchasing Document"
...
def test_analytical_fixture_dimensions_and_measures_non_empty():
    ...
    for expected in ("EBELN", "MATNR", "PLANT"):
...
def test_analytical_fixture_dimension_labels_populated():
    ...
    assert sem_by_name["EBELN"]["label"] == "Purchasing Document Number"
    assert sem_by_name["MATNR"]["label"] == "Material Number"
