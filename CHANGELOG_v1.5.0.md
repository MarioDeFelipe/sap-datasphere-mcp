...
Observed on the partstown-1 tenant (`SCM_BUSINESS / OPEN_PO_DETAIL`) before
the fix:
...
- Real CSDL fixtures captured live from partstown-1
  (`tests/fixtures/odata_v4_{analytical,relational}_OPEN_PO_DETAIL.xml`).
- `EBELN`/`EBELP` key preservation and labelling on the relational model.
...
```python
get_relational_metadata(space_id="SCM_BUSINESS", asset_id="OPEN_PO_DETAIL")
# before: every column.label is null
# after:  column.label populated where the tenant publishes one
#         (e.g. EBELN → "Purchasing Document Number")

get_analytical_metadata(space_id="SCM_BUSINESS", asset_id="OPEN_PO_DETAIL")
