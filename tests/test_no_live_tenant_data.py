...
NOTE: verify with filesystem reads, never ``git grep -E '\\b...'``. git grep uses
POSIX ERE, where ``\\b`` is not a word boundary, so such a pattern silently
reports clean on a file that plainly contains the token. That false negative is
how DEMO/SITE1 survived the first history rewrite.
...
_DENIED_B64 = [
    ...
    "T1BFTl9QT19RVFk=",
    "UE9MSU5FQ0hBTkdEVA==",
    "REVMSVZFUllUSU1FSU5EQVlT",
    "UFJPRFVDVEhJRVJBUkNIWQ==",
    "SU5GT1JFQ19QUklDRQ==",
    "RVhQRUNURURfREVMX0RU",
    "SVRFTV9ERUxfRFQ=",
    "UE9fQ1JFQVRJT05fRFQ=",
    "UE9fRE9DX1RZUEU=",
    "R1JfUVRZ"
...
def test_no_live_tenant_identifier_is_committed(token):
    # Word-boundary matched. Python's re supports \b correctly -- git grep -E
    # and BSD grep -E do not, and both silently reported clean during this
    # remediation. Boundaries also stop our tokens matching SAP's own shipped
    # content, e.g. PRODUCTHIERARCHY inside SAP_SC_FI_SQL_ProductHierarchy.
